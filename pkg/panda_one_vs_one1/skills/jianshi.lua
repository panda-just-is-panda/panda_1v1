...
Fk:loadTranslationTable{
  ["pang__jianshi"] = "舰势",
  [":pang__jianshi"] = "准备阶段，你可以依次重铸至多两张牌，然后你可以依次使用至多X张因此获得的【杀】（X为你装备区内的牌数）。",

  ["#jianshi_recast1"] = "舰势：你可以依次重铸至多两张牌（第1张）",
  ["#jianshi_recast2"] = "舰势：你可以依次重铸至多两张牌（第2张）",
  ["@@jianshi_slash-phase"] = "舰势",
  ["#jianshi_use1"] = "舰势：你可以使用至多两张因此获得的【杀】（第1张）",
  ["#jianshi_use2"] = "舰势：你可以使用至多两张因此获得的【杀】（第2张）",
...
jianshi:addEffect(fk.EventPhaseStart, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:recastCard(card1, player, jianshi.name,nil,"@@jianshi_slash-phase")
    ...
    if #card2 > 0 then
        room:recastCard(card2, player, jianshi.name,nil,"@@jianshi_slash-phase")
    end
    local slashes = table.filter(player:getCardIds("h"),function (element, index, array)
      return Fk:getCardById(element).trueName == "slash" 
      and Fk:getCardById(element):getMark("@@jianshi_slash-phase") > 0
    end)
    if #player:getCardIds("e") > 0 then
        ...
        slashes = table.filter(slashes, function (id)
